...
## Features
SwiftGLTFは、glTFに特化した**高性能なMetal駆動のレンダリングシステム**を提供します。 
SceneKitに依存せず、**幅広いglTF機能をサポート**し、アプリ向けに**軽量で統合しやすいSwiftパッケージ**として設計されています。  
Metalの低レベルGPU機能を活用することで、SwiftGLTFはAppleプラットフォーム上でglTFモデルの**高速・効率的かつ視覚的に正確なレンダリング**を実現します。

### 将来的な機能
- さらなるglTFの機能のサポート（詳しくは以下）
- VRMフォーマットのサポート
など
