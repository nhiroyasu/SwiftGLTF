...
func assertEqual(output: MTLTexture, goldenName: String) {
    ...
    goldenContext.draw(
        image,
        in: CGRect(x: 0, y: 0, width: goldenWidth, height: goldenHeight)
    )
    ...
    if mismatchCount > maxAllowedMismatchedPixels, !isCI() {
        do {
            try export(texture: output, name: "failed_\(goldenName).png")
        } catch {
            print("Failed to export failed texture: \(error)")
        }
    }

    #expect(
        mismatchCount <= maxAllowedMismatchedPixels,
        Comment(stringLiteral: "Output differs from golden image \(goldenName).png " +
                "with \(mismatchCount) mismatched bytes (tolerance=\(tolerance))")
    )
}
